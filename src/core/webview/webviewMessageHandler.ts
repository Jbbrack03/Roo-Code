...
import { getLmStudioModels, getLmStudioModelInfo } from "../../api/providers/lmstudio"
...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "requestLmStudioModels":
			...
			break
		case "getLmStudioModelInfo":
			if (message.baseUrl && message.modelId) {
				try {
					const modelInfo = await getLmStudioModelInfo(message.baseUrl, message.modelId)
					provider.postMessageToWebview({ 
						type: "lmStudioModelInfo", 
						modelInfo: modelInfo || { contextWindow: null } 
					})
				} catch (error) {
					console.error("Error fetching LM Studio model info:", error)
					provider.postMessageToWebview({ 
						type: "lmStudioModelInfo", 
