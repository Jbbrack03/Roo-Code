...
export class ClineProvider extends EventEmitter<ClineProviderEvents> implements vscode.WebviewViewProvider {
	...
	*/
	...
	public async getStateToPostToWebview() {
		const {
			apiConfiguration,
		...
		// Capture LM Studio model info for correct context window UI
		const currentCline = this.getCurrentCline()
		let lmStudioModelInfo: { contextWindow: number | null } | undefined = undefined
		if (currentCline?.api) {
			// Handle async or sync getModel()
			const result = currentCline.api.getModel()
		...
		if (currentCline) {
			// Use the ClineMessage history (ask + say) for token counting
			const chatHistory = currentCline.clineMessages || []
			// Only include user asks and assistant says
			...
			// Remove JSON, XML-like, or commit-hash-only blocks
			const filteredBlocks = chatBlocks.filter(b => {
				const t = b.text.trim()
				// Skip empty, JSON, or XML blocks
				if (!t || t.startsWith('{') || t.startsWith('<')) {
					return false
				}
				// Skip commit hashes (hex strings of length >=7)
				if (/^[0-9a-f]{7,}$/i.test(t)) {
					return false
				}
				return true
			})
		...
		return {
			...
			// Expose LM Studio context window size for UI
			lmStudioModelInfo,
			contextTokens,
		}
	}
