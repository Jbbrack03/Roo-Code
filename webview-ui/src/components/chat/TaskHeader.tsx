...
const TaskHeader = ({
	...
	const lmStudioModelInfo = state.lmStudioModelInfo
	const { info: model } = useSelectedModel(apiConfiguration)
	const [isTaskExpanded, setIsTaskExpanded] = useState(false)

	const textContainerRef = useRef<HTMLDivElement>(null)
	const textRef = useRef<HTMLDivElement>(null)
	const baseContextWindow = model?.contextWindow || 1
	const contextWindow =
		apiConfiguration.apiProvider === "lmstudio" && lmStudioModelInfo?.contextWindow != null
			? lmStudioModelInfo.contextWindow
			: baseContextWindow
				...
				{!isTaskExpanded && contextWindow > 0 && (
					<div className={`w-full flex flex-row gap-1 h-auto`}>
						<ContextWindowProgress
							...
							onlyCurrent={apiConfiguration.apiProvider === "lmstudio"}
						/>
