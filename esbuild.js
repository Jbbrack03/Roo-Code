...
const copyWasmFiles = {
	...
	setup(build) {
		build.onEnd(() => {
			...
			// Handle tiktoken WASM file with explicit error handling
			try {
				const tiktokenWasmSource = path.join(nodeModulesDir, "tiktoken", "tiktoken_bg.wasm")
				const tiktokenWasmDest = path.join(distDir, "tiktoken_bg.wasm")
				
				if (fs.existsSync(tiktokenWasmSource)) {
					console.log(`Copying tiktoken_bg.wasm to ${tiktokenWasmDest}`)
					fs.copyFileSync(tiktokenWasmSource, tiktokenWasmDest)
					// Ensure file is readable
					fs.chmodSync(tiktokenWasmDest, 0o644)
				} else {
					console.error(`tiktoken_bg.wasm not found at: ${tiktokenWasmSource}`)
				}
			} catch (error) {
				console.error("Error copying tiktoken_bg.wasm:", error)
			}

			// Main tree-sitter WASM file with explicit error handling
			try {
				const treeSitterWasmSource = path.join(nodeModulesDir, "web-tree-sitter", "tree-sitter.wasm")
				const treeSitterWasmDest = path.join(distDir, "tree-sitter.wasm")
				
				if (fs.existsSync(treeSitterWasmSource)) {
					console.log(`Copying tree-sitter.wasm to ${treeSitterWasmDest}`)
					fs.copyFileSync(treeSitterWasmSource, treeSitterWasmDest)
					// Ensure file is readable
					fs.chmodSync(treeSitterWasmDest, 0o644)
				} else {
					console.error(`tree-sitter.wasm not found at: ${treeSitterWasmSource}`)
				}
			} catch (error) {
				console.error("Error copying tree-sitter.wasm:", error)
			}
